...
public class DynamicMovingSpeed implements Listener {
	...
	static {
		...
		addModifier(0.13F, Material.SNOW);
		addModifier(0.17F, Material.SNOW_BLOCK);
		addModifier(0.18F, Material.FARMLAND, Material.GRAVEL, Material.SUSPICIOUS_GRAVEL);
		addModifier(0.18F, Tag.SAND.getValues());
		addModifier(0.22F, Tag.WOOL.getValues());
		addModifier(0.22F, Tag.WOOL_CARPETS.getValues());
		addModifier(0.22F, Tag.PLANKS.getValues());
		addModifier(0.22F, Tag.WOODEN_STAIRS.getValues());
		addModifier(0.22F, Tag.WOODEN_SLABS.getValues());
		addModifier(0.23F, Material.ICE, Material.BLUE_ICE, Material.PACKED_ICE, Material.FROSTED_ICE);
		addModifier(0.25F, Material.MAGMA_BLOCK);
		addModifier(0.27F,
		...
		addModifier(0.3F, Material.DIRT_PATH);
	}
	...
	public void onMove(EntityMoveEvent event) {
		...
		float speed = calculateSpeed(event.getTo().getBlock());
		double walkSpeed = attribute.getBaseValue();
		double diff = speed - walkSpeed;
		attribute.removeModifier(MODIFIER_KEY);
		attribute.addTransientModifier(new AttributeModifier(MODIFIER_KEY, diff, AttributeModifier.Operation.ADD_NUMBER));
	}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (player.isSwimming()) {
			MetadataUtil.setMetadata(player, BLOCK_SPEED_META, 0.2F);
			return;
		}

		Block block = event.getTo().getBlock();
		MetadataUtil.setMetadata(player, BLOCK_SPEED_META, calculateSpeed(block));
	}
	...
	public void onSpeedChange(PlayerMoveEvent event) {
		...
		if (!player.hasMetadata(BLOCK_SPEED_META))
			MetadataUtil.setMetadata(player, BLOCK_SPEED_META, calculateSpeed(event.getTo().getBlock()));

		AttributeInstance attribute = player.getAttribute(Attribute.MOVEMENT_SPEED);
		if (attribute == null) return;

		float walkSpeed = player.getWalkSpeed();
		float speed = MetadataUtil.getMetadata(player, BLOCK_SPEED_META, 0.2F).asFloat();
		if (walkSpeed != 0.2F) speed *= walkSpeed / 0.2F;

		double targetBonus = (speed - walkSpeed) / 2;

		double step = 0.0025;
		boolean abort = Math.abs(targetBonus) < step;
		if (abort) targetBonus = 0;
		...
		if (modifier != null) {
			...
			if (Math.abs(speedBoost - targetBonus) < step) {
				if (abort) return;
			} else {
				speedBoost += speedBoost > targetBonus ? -step : step;
			}
		} else {
			if (abort) return;
			speedBoost = targetBonus < 0 ? -step : step;
		}

		attribute.addTransientModifier(new AttributeModifier(MODIFIER_KEY, speedBoost, AttributeModifier.Operation.ADD_NUMBER));
	}

	public float calculateSpeed(Block block) {
		float slowness = 0F;
		...
		float target = SPEED_MODIFIERS.getOrDefault(downBlock.getType(), 0.2F);

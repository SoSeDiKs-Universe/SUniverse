...
@NullMarked
public class AttackZombieWithAnEggAdvancement extends BaseAdvancement {

	public AttackZombieWithAnEggAdvancement(BaseAdvancementBuilder<?, ?> advancementBuilder) {
	...
	private static AdvancementReward getReward() {
		return new FancyAdvancementReward().withExtraAction(action -> {
			...
			for (Material type : Tag.ITEMS_EGGS.getValues()) {
				...
				InventoryUtil.addOrDrop(completer, ItemStack.of(type, 16), false);
				return;
			}
		});
	}

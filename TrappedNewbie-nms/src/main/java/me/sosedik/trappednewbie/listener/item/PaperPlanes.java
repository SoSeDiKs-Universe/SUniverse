...
public class PaperPlanes implements Listener {
	...
	public void onLand(ProjectileHitEvent event) {
		if (!(event.getEntity() instanceof PaperPlane paperPlane)) return;
		if (paperPlane.getFireTicks() <= 0) return;
		...
		if (event.getHitBlock() != null) {
			...
			BurningArrowCreatesFire.createFire(block, blockFace);
		} else if (event.getHitEntity() != null) {
			Entity entity = event.getHitEntity();
			entity.setFireTicks(Math.max(entity.getFireTicks(), 20 * 5));
		}
	}

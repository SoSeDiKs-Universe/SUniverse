...
@NullMarked
public class KillHostileNetherMobsAdvancement extends BaseAdvancement {

	public KillHostileNetherMobsAdvancement(BaseAdvancementBuilder<?, ?> advancementBuilder) {
		super(advancementBuilder.requiredProgress(getProgress()));
	}

	private static RequiredAdvancementProgress getProgress() {
		List<List<String>> requirements = new ArrayList<>(UtilizerTags.NETHER_HOSTILE_MOBS.getValues().size());
		List<VanillaTriggerData<?>> triggerDatas = new ArrayList<>();
		for (EntityType type : UtilizerTags.NETHER_HOSTILE_MOBS.getValues()) {

...
@NullMarked
public class KillHostileOverworldNightMobsAdvancement extends BaseAdvancement {

	public KillHostileOverworldNightMobsAdvancement(BaseAdvancementBuilder<?, ?> advancementBuilder) {
		super(advancementBuilder.requiredProgress(getProgress()));
	}

	private static RequiredAdvancementProgress getProgress() {
		List<List<String>> requirements = new ArrayList<>(UtilizerTags.OVERWORLD_HOSTILE_NIGHT_MOBS.getValues().size());
		List<VanillaTriggerData<?>> triggerDatas = new ArrayList<>();
		for (EntityType type : UtilizerTags.OVERWORLD_HOSTILE_NIGHT_MOBS.getValues()) {
	...
	private static boolean shouldRequireDimension(EntityType type) {
		return type == EntityType.SKELETON || type == EntityType.ENDERMAN;
	}
	...
	private static PlayerKilledEntityTriggerData triggerDataWithDimension(EntityType type) {
		...
		((EntityTriggerCondition) first).withLocation(loc -> loc.withDimension(World.Environment.NORMAL));
		return triggerData;
	}

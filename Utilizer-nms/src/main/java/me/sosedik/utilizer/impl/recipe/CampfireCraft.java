...
/**
 * Builder for {@link CampfireRecipe}
 */
@NullMarked
public class CampfireCraft extends BurningRecipeBuilder<CampfireCraft> {

	private CookingBookCategory category;

	public CampfireCraft(ItemStack result, int time, NamespacedKey key) {
		super(result, time, new NamespacedKey(key.getNamespace(), key.value() + "_from_campfire_cooking"));
		this.category = CookingBookCategory.MISC;
	}

	@Override
	public CampfireCraft register() {
		var recipe = new CampfireRecipe(getKey(), getResult(), getRecipeChoice(), getExp(), getTime());
	 ...
	 * @return this builder
	 */
	public CampfireCraft withCategory(CookingBookCategory category) {
